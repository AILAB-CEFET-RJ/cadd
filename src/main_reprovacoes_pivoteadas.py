...
def processar_reprovacoes(planilha, outputdir):
    ...
    qtds_alunos = list(dados.values())

    plotar_grafico(sigla_curso, outputdir, qtds_reprovacoes, qtds_alunos)


def plotar_grafico(sigla_curso, outputdir, lista_qtd_reprovacoes, lista_qtd_alunos):
    lista_qtd_reprovacoes_str = ['= ' + str(x) for x in lista_qtd_reprovacoes]
    if lista_qtd_reprovacoes_str:
    ...
    layout_quantidades = go.Layout(
        title=f'Curso: {sigla_curso}<br>"Quantidades de alunos" versus "Qtd. máxima de reprovações em alguma disciplina"',
    ...
    filename = os.path.join(outputdir, f"{sigla_curso}-reprovacoes.html")
